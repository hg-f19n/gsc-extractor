...
module.exports.run = async (page, siteUrl, markdownFilePath) => {
  try {
    ...
    /**
     * Get all available Experience Section URLs
     */
    url = `https://search.google.com/search-console?resource_id=${encodeURIComponent(siteUrl)}`;
    await navigateToUrl(page, url);

    const subSectionXPath = "//div[contains(., 'Experience') and @class and @jsname]/following-sibling::div//div[@data-scfe-feature]/a";
    let subSectionElements = await page.$x(subSectionXPath);

    // If the Experience section does not exist, log to console and exit
    ...
    for (let subSectionUrl of subSectionUrls) {
      ...
      result = await captureScreenshot(page, siteUrl, screenshotXPath,  `experience-${headingTextSanitized}`);
      await markdown.generateMarkdownSlide(`Experience - ${headingText}`, result.screenshotPath, result.pageUrl, markdownFilePath);
      await sleep(500);
    }

  } catch (error) {
    console.error(`Error in experience.run: ${error}`);
  }
};
